...
export default function Sidebar({
  ...
  const [deletingSession, setDeletingSession] = useState<SidebarSessionItem | null>(null);
  ...
  const handleRename = async () => {
    ...
    if (!normalized || normalized === renamingSession.title.trim()) {
      setRenamingSession(null);
      setRenameValue('');
      setMenuSessionId(null);
      return;
    }
    await onRenameSession(renamingSession.id, normalized);
    setRenamingSession(null);
    setRenameValue('');
    setMenuSessionId(null);
  };
  ...
  return (
    <>
                      ...
                      <div key={item.id} className="group relative">
                        ...
                        {!collapsed && menuSessionId === item.id && (
                          <div className={`absolute right-2 top-[calc(100%+4px)] z-20 w-36 overflow-hidden rounded-2xl border shadow-xl ${tone.menuBox}`}>
                            <button
                              onClick={() => {
                                setRenamingSession({ id: item.id, title: item.title || '未命名会话' });
                                setRenameValue(item.title || '未命名会话');
                              }}
      ...
      {renamingSession && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm" onClick={() => { setRenamingSession(null); setRenameValue(''); }}>
          <div className="cg-modal w-full max-w-sm rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="text-base font-semibold text-slate-800 dark:text-slate-100">重命名会话</div>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">给这个会话换一个更容易识别的名字。</p>
            <input
              value={renameValue}
              onChange={(event) => setRenameValue(event.target.value)}
              className="mt-4 w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-200"
              placeholder="输入会话名称"
              autoFocus
            />
            <div className="mt-4 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => { setRenamingSession(null); setRenameValue(''); }}
              ...
              <button
                ...
                className="rounded-xl bg-sky-500 px-3 py-2 text-sm font-medium text-white hover:bg-sky-600"
              >
                保存
              </button>
            </div>
      ...
      {deletingSession && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm" onClick={() => setDeletingSession(null)}>
          <div className="cg-modal w-full max-w-sm rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="text-base font-semibold text-slate-800 dark:text-slate-100">删除会话</div>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">确定要删除“{deletingSession.title || '未命名会话'}”吗？删除后无法恢复。</p>
            <div className="mt-4 flex justify-end gap-2">
              <button
                type="button"
