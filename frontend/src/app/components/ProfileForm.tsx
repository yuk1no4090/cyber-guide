...
export default function ProfileForm({ initialValue, onSubmit }: ProfileFormProps) {
  ...
  return (
    <div className="message-bubble flex justify-start mb-3">
      <div className="polish-card max-w-[96%] sm:max-w-[86%] rounded-2xl rounded-bl-sm overflow-hidden">
        <div className="bg-gradient-to-r from-sky-50 via-blue-50 to-sky-50 dark:from-slate-900 dark:via-sky-950/40 dark:to-slate-900 border border-sky-200 dark:border-sky-900/50 rounded-t-2xl px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[14px] font-semibold text-sky-700 dark:text-sky-200">先填一下你的画像，我会给更贴身建议</span>
          </div>
        </div>
        <div className="ai-bubble rounded-t-none border-t-0 px-4 py-4 space-y-3">
          ...
          <button
            ...
            disabled={!canSubmit}
            className="w-full py-2 text-[13px] font-medium text-sky-700 dark:text-sky-100 bg-sky-50 dark:bg-sky-900/45 border border-sky-200 dark:border-sky-800 rounded-xl hover:bg-sky-100 dark:hover:bg-sky-900/70 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
          >
            开始分析
          </button>
        </div>
