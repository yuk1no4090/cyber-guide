...
### 1.4 登录用户与数据库持久化（与「改进用案例卡片」不同）

- **已登录用户**：对话会话与消息、画像 JSON、7 日计划相关行等保存在 **PostgreSQL** 中，用于跨设备/刷新后继续使用；后端会对敏感片段做脱敏策略（与产品实现一致）。
- **匿名用户**：仍可使用完整对话能力；服务端对匿名会话的延续策略以 `ChatPersistenceService` 为准（新建对话应避免挂到旧匿名会话上）。
- 本节下文所述 **opt-in 案例卡片 / JSONL** 仍为「产品改进」可选通道，与账号业务数据存储方式独立；若未实现文件落盘，以实际代码与部署为准。
