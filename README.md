...
cd cyber-guide
cp .env.example .env   # edit .env, set OPENAI_API_KEY; defaults target a Zhipu OpenAI-compatible endpoint
...
AI 接口说明：
- 当前环境变量沿用 OpenAI-compatible 命名：`OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL`
- 默认 `.env.example` 和后端配置指向智谱兼容端点 `https://open.bigmodel.cn/api/paas/v4` 与 `glm-4-flash`
- 若切换到其他兼容供应商，只需替换 base URL、API key 和 model
